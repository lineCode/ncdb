/*******************************************************************************
*  @file      UIIMList.cpp 2016-11-18 15:18:16
...
Node* UIIMList::AddNode(const IMListItemInfo& item, Node* parent, int index)
{
	...
	if (!m_dlgBuilder.GetMarkup()->IsValid())
	{
		...
		pListElement = static_cast<CListContainerElementUI*>(m_dlgBuilder.Create(_T("recentlyListItem.xml"), (UINT)0, NULL, &paint_manager_));
	}
	else
	{
...
Node* UIIMList::GetItemBySId(IN const int sId)
{
	if (!root_node_)
		return 0;
...
BOOL UIIMList::IsExistSId(IN const int sId)
{
...
int UIIMList::_reArrange(Node *node, int iLastIndex)
{
	if (!node)
		return iLastIndex;
	...
	if (!node->folder())
	{
		...
		CControlUI *pControl = node->data().list_elment_;

		SetItemIndex(pControl, iLastIndex);
	...
	int iChildren = node->num_children();
	for (int i = 0; i < iChildren; i++)
	{
		Node *child = node->child(i);

		iLastIndex = _reArrange(child, iLastIndex);
	}

	return iLastIndex;
}

void UIIMList::SortChildren(Node *parent, std::function<bool(Node *&x, Node *&y)> compare)
{
	if (!parent || !parent->folder()) return;
	if (!parent->has_children()) return;

	parent->sort_children(compare);

	CListContainerElementUI *pListElement = parent->data().list_elment_;

	int iLastIndex = (NULL == pListElement) ? -1 // root node
		: pListElement->GetIndex();

	int iChildren = parent->num_children();
	for (int i = 0; i < iChildren; ++i)
	{
