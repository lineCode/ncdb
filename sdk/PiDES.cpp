...
// DES加密
//
bool CPiDES::DES_Encrypt ( LPBYTE lpSour, LPBYTE lpDest, UINT uLen, LPBYTE lpKey )
{
	if ( ( uLen & 7 ) > 0 )
		return false ;
...
UNINT CPiDES::DES_EncryptEx( LPBYTE lpSour, LPBYTE& lpDest, UNINT uLen, LPBYTE lpKey )
{
	int nLeft = (uLen & 7);
	UINT nLenNew = uLen;
	byte* cTemp = lpSour;
	...
	{
		//追加0字符
		nLenNew = uLen + (8 - nLeft);
		cTemp = new byte[nLenNew];
		memset(cTemp, 0, nLenNew);
		memcpy_s(cTemp, nLenNew, lpSour, uLen);
	}
	lpDest = new BYTE[nLenNew];
	memset(lpDest, 0, nLenNew);

	bool bRet = DES_Encrypt(cTemp, lpDest, nLenNew, lpKey);
	if (uLen != nLenNew)
	{
		delete[] cTemp;
	}

	return bRet ? nLenNew: 0;
}

UNINT CPiDES::DES_EncryptEx( LPBYTE lpSour, LPBYTE& lpDest, UNINT uLen, const char* lpKey )
{
	return DES_EncryptEx(lpSour, lpDest, uLen, (LPBYTE)lpKey);
}

//
// DES解密
//
bool CPiDES::DES_Decrypt( LPBYTE lpSour, LPBYTE lpDest, UINT uLen, LPBYTE lpKey )
{
	if ( ( uLen & 7 ) > 0 )
		return false ;
	memset ( this->bKey, 0, sizeof(this->bKey) ) ;
...
UNINT CPiDES::DES_DecryptEx( LPBYTE lpSour, LPBYTE& lpDest, UINT uLen, LPBYTE lpKey )
{
	int nLeft = (uLen & 7);
	UINT nLenNew = uLen;
	nLenNew += (nLeft ? (8 - nLeft): 0);
	lpDest = new BYTE[nLenNew];
	memset(lpDest, 0, nLenNew);
	if(! DES_Decrypt(lpSour, lpDest, nLenNew, lpKey))
	{
		return 0;
	}
	return nLenNew;
...
UNINT CPiDES::DES_DecryptEx( LPBYTE lpSour, LPBYTE& lpDest, UINT uLen, const char* lpKey )
{
	return DES_DecryptEx(lpSour, lpDest, uLen, (LPBYTE)lpKey);

}

UNINT CPiDES::DES_DecryptEx( LPBYTE lpSour, string& lpDest, UINT uLen, const char* lpKey )
{
	LPBYTE pDecrypt = nullptr;
	UNINT nLenDecode = DES_DecryptEx(lpSour, pDecrypt, uLen, lpKey);
	lpDest.assign((char*)pDecrypt, nLenDecode);
	ClearMemArr(pDecrypt);
	return nLenDecode;
}
...
// DES的单轮加密过程
//
void CPiDES::DESSingleTurn ( BYTE nTurnIndex, bool bMode )
{
	// L[n] = R[n-1]
	for ( UINT i = 0; i < 4; i++ )
...
// DES的F函数
//
void CPiDES::DES_f ( BYTE bTurnIndex, bool bMode )
{
...
void CPiDES::DES_SetOriMsg ( PBYTE pMsg, UINT uLen )
{
	memset ( this->bOriMsg, 0, sizeof(this->bOriMsg) ) ;
	memcpy ( this->bOriMsg, pMsg, uLen ) ;
}
void CPiDES::DES_SetKey ( PBYTE pKey, UINT uLen )
{
	memset ( this->bKey, 0, sizeof(this->bKey) ) ;
