...
void WeiboTestCase::standardOptionForWaiting(unsigned int id)
{
	while(1)
	{
		...
		if (result)
		{
			const WeiboTestCaseResult::eResultType retType = result->resultType_;
			// Must check is oauth succeed.
			if (WBOPT_OAUTH2_ACCESS_TOKEN == id && 
				WeiboTestCaseResult::ERT_UNKNOW != retType && 
				result->pasringPtr_)
			{
				bool isSucceed = false;
				
				if (WeiboTestCaseResult::ERT_CORRECT == retType)
				{
					ParsingOauthRetPtr oauthRet = 
						boost::dynamic_pointer_cast<ParsingOauthRet>(result->pasringPtr_);

					isSucceed = (oauthRet && 
						!oauthRet->access_token.empty() &&
						!oauthRet->uid.empty());
				}
				...
				if (!isSucceed)
				{
					// Oauth failed
					assert(0);
					CPPUNIT_ASSERT_MESSAGE("Oauth failed!!! Please check your user name and password, Or check error information!", 0);
				}
				continue;
			}
...
void WeiboTestCase::oauth2()
{
	...
	code = "0174a561058f265a9a7788a4d2bccf67";
	cin >> code;
