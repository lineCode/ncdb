...
CWndShadow::pfnDwmSetWindowAttribute CWndShadow::s_DwmSetWindowAttribute = NULL;
...
bool CWndShadow::Initialize(HINSTANCE hInstance)
{
	...
	if (hSysDll)	// Loaded dwmapi.dll succefull, must on Vista or above
	{
		...
		s_DwmSetWindowAttribute =
			(pfnDwmSetWindowAttribute)GetProcAddress(hSysDll,
...
void CWndShadow::Create(HWND hParentWnd)
{
	...
	if (bAero)
	{
		if (s_DwmSetWindowAttribute != NULL)
		{
			LPCVOID dwAttribute = (LPCVOID)TRUE;

			s_DwmSetWindowAttribute(hParentWnd, 2,
				&dwAttribute, sizeof(dwAttribute));
		}
		else
		{//Disable Aero in Windows Vista.
...
void CWndShadow::Update(HWND hParent)
{
	...
	RECT WndRect = {0};
	if(!GetWindowRect(hParent, &WndRect))
	{
		return ;
	}
...
void CWndShadow::Show( HWND hParent, bool bWillShow /*= false*/ )
{
	...
	if ((m_Status & SS_ENABLED) && !(m_Status & SS_DISABLEDBYAERO))	// Enabled
	{
		...
		if (WS_VISIBLE & lParentStyle || bWillShow)	// Parent visible
		{
