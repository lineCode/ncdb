...
// 
////////////////////////////////////////////////////////////////////////////////
ImageEx::ImageEx(LPCTSTR filename, BOOL useEmbeddedColorManagement) : Image(filename, useEmbeddedColorManagement)
{
	Initialize();
	...
	if (IsAnimatedGIF())
	{
		if (m_hThread == NULL)
		{
			...
			m_hThread = ::CreateThread(
				NULL,
				0,
				(LPTHREAD_START_ROUTINE)&ImageEx::_ThreadAnimationProc,
				this,
				0,
				&threadID	
				);
...
#else
	m_hInst = NULL;// AfxGetResourceHandle();
#endif
...
bool ImageEx::ShowNextFrame( HDC hdcUI )
{
	GUID   pageGuid = FrameDimensionTime;
	Graphics graphics(hdcUI);
	graphics.DrawImage(this, m_pt.x, m_pt.y, GetWidth(), GetHeight());
	//graphics.DrawImage(this, m_pt.x, m_pt.y, hmWidth, hmHeight);
...
bool ImageEx::IsPicOk()
{
	return m_nFrameCount > 0;
}

void ImageEx::SetDrawPoint( ::CPoint pt )
{
	m_pt = pt;
}

int ImageEx::GetFrameDelay( int param1 )
{
	if(!m_pPropertyItem)
	{
		return 100;
